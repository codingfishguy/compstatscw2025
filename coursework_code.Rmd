...
```{r}
shifted_exponential <- function(x) {
  if (x>=2){
    return (exp(-0.02*(x-2)))
  }
  else{
...
plot(t, sapply(t, function(t) shifted_exponential(t)), type="l", xlab="x", ylab = "f(x)") #seems like a relatively good way to do my independence sampler

#can maybe sample from it via inversion method

shifted_samples <- function(lambda){
  u <- runif(1)
  return ((2*lambda - log(exp(2*lambda)-u)/lambda))
} 
...
MH_newprop <- function(X0, nsteps, f){
  ...
  X[1] <- X0
  for (i in 2:(nsteps+1)){
    Y <- shifted_samples(0.1)
    if (log(runif(1))<=min(log( (f(Y)*shifted_exponential(Y))/(f(X[i-1])*shifted_exponential(X[i-1]))),1)){
      X[i] <- Y}
    else{ X[i] <- X[i-1]}
   }
 X
