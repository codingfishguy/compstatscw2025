...
I implemented the empirical risk as a function of my model predictions
and actual realisations of the data (with quantile $\tau$ as an
additional argument). I then created a loss function to minimise, which
was a function of parameters $\beta_0$, $\beta_1$, $\beta_2$, and
$\tau$, where predictions were generated through a baseline linear model
(with parameters as specified), using lstat and rm in my training data
as predictors, and then the risk of these predictions compared to the
actual realisation of the data (medv) was computed.

For the initial state for my parameters $\beta_0$, $\beta_1$, $\beta_2$
I chose $(0,-5,5)$. I chose a negative value for lstat as I felt it made
...
higher house prices. I used the default optimisation method from the
optim function to then optimise for my parameter values by minimising
the loss function for each value of $\tau$, which led to the following
estimates. I assessed convergence using the convergence part of the
optim function; as can be seen, it was 0 for all values of $\tau$,
suggesting successful convergence.
...
results <- matrix(0, nrow = length(tau), ncol = 4)

for (i in seq_along(tau)) {
  t <- tau[i]
  opt <- optim(par = c(0, -5, 5), fn = riskfun, t = t)

  ## store parameters + convergence code
  results[i, ] <- c(round(opt$par, 2), opt$convergence)
}

## add column names
colnames(results) <- c("B0", "B1", "B2", "Convergence")
rownames(results) <- paste0("tau_", tau)
...
The Empirical Loss was then calculated (on the test set) to be:

```{r}
#| echo: false

#Empirical Loss on Test Set
riskfun2 <- function(par, t){
  B0 <- par[1]
  B1 <- par[2]
  B2 <- par[3]
...
losses <- c(
  riskfun2(c(results[1,1], results[1,2], results[1,3]), 0.1),
  riskfun2(c(results[2,1], results[2,2], results[2,3]), 0.5),
  riskfun2(c(results[3,1], results[3,2], results[3,3]), 0.9)
)
...
#t=0.1
emp_cov(c(results[1,1], results[1,2], results[1,3]))

#t=0.5
emp_cov(c(results[2,1], results[2,2], results[2,3]))

#t=0.9
emp_cov(c(results[3,1], results[3,2], results[3,3]))
```
...
riskfun3 <- function(par, t){
  ...
  predictions <- B0 + B1*train_dat$lstat + B2*train_dat$rm + 
    B3*(train_dat$lstat)^2 + B4 * (train_dat$rm)^2 + 
    B5* (train_dat$rm * train_dat$lstat)
  
  actuals <- train_dat$medv #i.e. the response
  return (risk(predictions, actuals, t))
}

results2 <- matrix(0, nrow = length(tau), ncol = 7)

for (i in seq_along(tau)) {
  t <- tau[i]
  opt <- optim(par = c(0,0,0,0,0,0), fn = riskfun, t = t)

  ## store parameters + convergence code
  results2[i, ] <- c(round(opt$par, 2), opt$convergence)
}
...
# Compute the empirical losses
losses <- c(
  riskfun4(c(results2[1,1], results2[1,2], results2[1,3], results2[1,4], results2[1,5], results2[1,6]), 0.1),
  riskfun4(c(results2[2,1], results2[2,2], results2[2,3], results2[2,4], results2[2,5], results2[2,6]), 0.5),
  riskfun4(c(results2[3,1], results2[3,2], results2[3,3],results2[3,4], results2[3,5], results2[3,6]), 0.9)
)
